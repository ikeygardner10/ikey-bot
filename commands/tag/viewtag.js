	...
	execute: async (client, message, args) => {
		...
		if(serverRows[0] !== undefined) {
				...
				if(serverRows[0].imageURL !== null) {
					...
					return fileGrab(fileURL)
						.then(() => {
							...
							return message.lineReply(serverRows[0].content, { files: [{ attachment: fileURL, name: fileName }] });
						})
						.catch((error) => {
							console.error(`[VIEW TAG] ${error.stack}`);
							console.error(`[VIEW TAG] Server tag image ${serverRows[0].imageURL} corrupted`);
							console.success(`[VIEW TAG] Server tag sent: ${ntn}`);
							return message.lineReply(serverRows[0].content + '\n\n*(file corrupted)*');
						});
				}
				else {
					console.success(`[VIEW TAG] Server tag sent: ${ntn}`);
			...
			if(globalRows[0] !== undefined) {
					...
					if(globalRows[0].imageURL !== null) {
						...
						return fileGrab(fileURL)
							.then(() => {
								...
								return message.lineReply(globalRows[0].content, { files: [{ attachment: fileURL, name: fileName }] });
							})
							.catch((error) => {
								console.error(`[VIEW TAG] ${error.stack}`);
								console.error(`[VIEW TAG] Global tag image ${globalRows[0].imageURL} corrupted`);
								console.success(`[VIEW TAG] Global tag sent: ${ntn}`);
								return message.lineReply(globalRows[0].content + '\n\n*(file corrupted)*');
							});
					}
					else {
						console.success(`[VIEW TAG] Global tag sent: ${ntn}`);
				...
				return message.lineReply(`:mag: No tag **${ntn}** found`);
			}
		}
