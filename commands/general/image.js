...
module.exports = {
	...
	execute: async (client, message, args) => {
		...
		if(!toSearch) return message.lineReply('`Invalid (SPECIFY SEARCH)`');
		...
		if(images.length === 0) return message.lineReply('`Invalid (NO RESULTS)`');
		if(images.error && images.error.code === 403) return message.lineReply('`Invalid (SEARCH QUOTA LIMIT)');
